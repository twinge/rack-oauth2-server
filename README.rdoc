...
If you are running Rails 3 or another Rack server and using ActiveRecord 3.0.x, this code _may_ work for you. Please submit issues and pull requests when you encounter problems.
...
An example of the migration you will want to perform is in sample_migration.rb at the root of the project.

=== Step 2: Use The Server

==== Rails 3.0

Configure Rack::OAuth2::Server by adding the following to your +config/application.rb+ (or one of the specific environment files). For example:
...
First require +rack/oauth2/sinatra+ and register
...
You can +use Rack::OAuth2::Server+ and pass your own {Rack::OAuth2::Server::Options} object.
...
typically the user ID or account ID, but can be anything you want, as long as
  ...
  class OauthController < ApplicationController
    def authorize
      if current_user
        ...
        # send the user to your application's login route
...
The view would look something like this:
  ...
  <h2>The application <% link_to h(oauth.client.display_name), oauth.client.link %>
    is requesting to <%= oauth.scope %> your account.</h2>
...
You can register clients using the command line tool +oauth2-server+:
  ...
  $ oauth2-server register --db db/development.sqlite3
...
The easiest way to do this is to run the +oauth2-server+ command line tool:

  $ oauth2-server setup --db db/development.sqlite3
...
The original Rack::OAuth2::Server was written to provide authorization/authentication for
...
Thanks to Tony Pitale[https://github.com/tpitale] for forking the original project and beginning the process of replacing Mongoid persistence with ActiveRecord persistence.
